...
const AccountScreen = ({ navigation }) => {
  ...
  const handleSaveEdit = async () => {
    try {
      // Actualiza en Firestore (ajusta la colección y el id según tu estructura)
      const userRef = doc(db, "usuarios", authState.user.uid);
      await updateDoc(userRef, editData);
      alert("Información actualizada correctamente.");
      closeEditModal();
      // Aquí deberías actualizar el estado global del usuario si es necesario
      // Por ejemplo: onUpdateUser(editData);
    } catch (e) {
